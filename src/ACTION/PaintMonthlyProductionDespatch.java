...
public class PaintMonthlyProductionDespatch {
	...
	public String str4;
	public String str5;
	public String getMonthlyProdDatapaint()
	{
		...
		String queryString="select A.fnyr, A.month_code, A.first_month_date, A.end_month_date,(select count(B.assembly_start_date) from paint_tran B where B.assembly_start_date  between A.first_month_date and  A.end_month_date ) as monthly_produced, (select count(C.assembly_dispatch_date) from paint_tran C where C.assembly_dispatch_date  between A.first_month_date and  A.end_month_date  ) as monthly_dispatched from public.fnyr_mon_start_end_date_v A where A.fnyr='"+str5+"'";
		queryForData = session.createSQLQuery(queryString);	
		String queryString2="select A.fnyr, A.month_code, A.first_month_date, A.end_month_date,(select count(B.assembly_start_date) from paint_tran B where B.assembly_start_date  between A.first_month_date and  A.end_month_date ) as monthly_produced, (select count(C.assembly_dispatch_date) from paint_tran C where C.assembly_dispatch_date  between A.first_month_date and  A.end_month_date  ) as monthly_dispatched from public.fnyr_mon_start_end_date_v A where A.fnyr='"+str4+"'";
