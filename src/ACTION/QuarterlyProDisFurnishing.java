...
public class QuarterlyProDisFurnishing {
	...
	public String str4;
	public String str5;
	public String getQuarterlyProdDataFurnish()
	{
		...
		String queryString="select A.fnyr, A.quarter, A.start_qtr_date, A.end_qtr_date,(select count(B.assembly_start_date) from furnishing_tran B where B.assembly_start_date  between A.start_qtr_date and A.end_qtr_date ) as quarterly_produced,(select count(C.assembly_dispatch_date) from furnishing_tran C where C.assembly_dispatch_date between A.start_qtr_date and  A.end_qtr_date  ) as quarterly_dispatched from public.fnyr_qtr_start_end_date_v A where A.fnyr='"+str5+"'";
		queryForData = session.createSQLQuery(queryString);	
		String queryString2="select A.fnyr, A.quarter, A.start_qtr_date, A.end_qtr_date,(select count(B.assembly_start_date) from furnishing_tran B where B.assembly_start_date  between A.start_qtr_date and A.end_qtr_date ) as quarterly_produced,(select count(C.assembly_dispatch_date) from furnishing_tran C where C.assembly_dispatch_date between A.start_qtr_date and  A.end_qtr_date  ) as quarterly_dispatched from public.fnyr_qtr_start_end_date_v A where A.fnyr='"+str4+"'";
