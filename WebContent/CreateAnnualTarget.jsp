...
<p>(Only number values are allowed)</p>
			...
			<tr> <th> Coach Type</th><th>Target Quantity</th>
		...
		<%
			...
			try {
				...
				// {
				sql = "select asset_subtype from public.asset_master where asset_type= 'Coach'";
				ResultSet rs = st.executeQuery(sql);
				while (rs.next()) {
					coach_type = rs.getString(1);
					String sql_qty = "select yr_coach_target from public.coach_target where coach_type='" + coach_type
   ...
   <%  if(yearEntered>=current_year)  //current_year
    {%>
    <td><input type="text" name="qty" value="<%=qty%>" size="20" onkeypress="return isNumberKey(event)" onblur="retrieveURL('CreateAnnualTarget_do.jsp?coach_type=<%=coach_type%>&financialYear=<%=financialYear%>&FIELD=yr_coach_target', this.value)"/></td>
  <%  }
   else
   { %>
   <td><%=qty%></td>
