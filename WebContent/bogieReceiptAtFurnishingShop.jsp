	...
	//toolbar code
	   {
                    ...
                    text: 'Receipt Bogieset',
                    click: function () {
                    	
                    	 var $selectedRows = $('#BogiesetProductionTableContainer').jtable('selectedRows');
                	 	 if ($selectedRows.length<1)
                		 alert("Please select a row for which you want to receipt Bogieset at Bogie Shop");
                	 	 else{
                	 		var record;
                	 		$selectedRows.each(function () {
                	 	 	...
                	 	 	$("#receiptDialog").dialog({
                	 	 		title : 'Receipt for   '+record.bogieNumber+'',                     	 	 		
                	 	 		close: function (e, u) {
          	 		              //$(".formError").remove();
                                    ...
                                    Submit: function () {
                                    	...
                                    	$.ajax({
                    						url : "receiptBogie",  //action-name as defined in struts.xml
                    						dataType: 'json', 
                    						data : {
				...
				assemblyExitDate: {
					...
					bogieNumber: {
						...
						bogieType: {
							...
							display: function (data)
							{
								if(data.record.bogieType=="coil_spring")
									return "COIL SPRING";
								if(data.record.bogieType=="air_spring")
									return "AIR SPRING";
							}
							},
